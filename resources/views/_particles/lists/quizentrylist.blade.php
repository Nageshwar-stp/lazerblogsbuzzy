...
<section class="entry quizquestion selectableQuest" id="section_{{ $entry->order }}" data-entry="{{ $entry->id }}">
    ...
    @if($entry->image)
    <div class="media">
        <a id="" class="gif-icon-a"><img class=" lazyload img-responsive" style="display: block;width:100%"
                alt="{{ $entry->title }}" data-src="{{ url(makepreview($entry->image, null, 'entries')) }}"></a>
            ...
            <li>
                <a class="" href="javascript:" data-answer="{{ $answers->id }}" data-result="{{ $answers->video }}">
                    <div class="answer-cover">
                        @if($entry->video!='3')
                        <img class=" lazyload responsive-img" alt="{{ $answers->title }}"
                            data-src="{{ url(makepreview($answers->image, null, 'answers')) }}">
                        @endif
