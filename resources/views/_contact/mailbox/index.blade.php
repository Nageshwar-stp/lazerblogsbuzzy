                ...
                <table class="table table-hover table-striped contactlist">
                    <tbody>
                        ...
                        <tr class=" {{ $mail->read==1 ?: 'unread' }}">
                            ...
                            <td style="width:59%;">
                                <a class="font-w600 taba"
                                    href="/admin/mailbox/read/{{ $mail->id }}">{{ $mail->subject }}</a>
                                <div class="text-muted push-5-t">{{ str_limit(strip_tags($mail->text), 120) }}..</div>
                            </td>
                            <td style="width:10%;">
                                @if(isset($mail->label))
                                <div class="label label-info"
                                    style="background-color: {{ $mail->label->description }} !important;">
                                    {{ $mail->label->name }}</div>
                                @endif
                            </td>
                        ...
                        <div class="alert alert-default alert-dismissible"
                            style="text-align: center;padding:170px 50px">

                            <h4><i class="icon fa fa-{{ $caticon }}"
                                    style="font-size:80px;margin-bottom:20px;opacity: 0.5"></i> <br>
                                <b>{{ $catname }}</b> {{  trans("buzzycontact.folderisempty") }}.!</h4>
