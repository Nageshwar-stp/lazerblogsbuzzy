...
<section class="entry quizquestion selectableQuest" id="section_{{ $entry->order }}" data-entry="{{ $entry->id }}">
    ...
    <div class="media">
        ...
        <a id="" class="gif-icon-a"><img class="img-responsive" style="display: block;width:100%"
                alt="{{ $entry->title }}" src="{{ makepreview($entry->image, null, 'entries') }}"></a>
...
<section class="entry results" id="quiz_result" data-popup="{{ get_buzzy_config('BuzzyQuizzesPopup') }}"
    ...
    <div class="quiz_result_area">
        ...
        @endif
        <ol>
            @foreach($post->entries()->byType("quizresult")->oldest("order")->get() as $keyp => $entry)
            <li class="quiz_result" data-order="{{ $keyp }}" data-result="{{ $entry->id }}"
                ...
                <div class="quiz_img {{ $entry->image == '' ? 'hide' :'' }}">
                    <img class="responsive_img" style=" float: right;"
    ...
    <div class="quiz_result_share">
        ...
        <div class="external-sign-in">
            ...
            <a href="https://plus.google.com/share?url={{ Request::url() }}"
                class="Google popup-action">{{trans('index.sharegoogle') }}</a>
        </div>
        <div class="clear"></div>
    </div>
</section>
