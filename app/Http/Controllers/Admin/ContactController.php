...
class ContactController extends MainAdminController
{
    public function __construct()
    {
        ...
        $mailcat = Tag::byType('mailcat')->oldest('id')->get();
        $mailprivatecat = Tag::byType('mailprivatecat')->oldest('id')->get();
        $mailsections = Tag::byType('maillabel')->oldest('id')->get();
    ...
    public function index($type = "")
    {
        ...
        if (!empty($type)) {
            $cat = Tag::where('slug', $type)->whereIn('type', ['mailcat', 'maillabel', 'mailprivatecat'])->first();
            if (!$cat) {
    ...
    public function addcat(Request $request)
    {
        ...
        $cat->color = sprintf('#%06X', mt_rand(0, 0xFFFFFF));
    ...
    public function maillabeldelete($id)
    {
        ...
        foreach (Contacts::where('label_id', $id)->get() as $getr) {
            $getr->label_id  = Tag::byType('maillabel')->first()->id;
            $getr->save();
        }
    ...
    public function doaction(Request $request)
    {
        ...
        foreach ($requests['contacts'] as $con) {
            ...
            if ($type == 'move') {
                $tagCat = Tag::whereIn('type', ['mailcat', 'mailprivatecat'])->where('slug', $typo)->first();
                if ($tagCat) {
                    $mail->category_id = $tagCat->id;
                }
            } elseif ($type == 'do') {
                if ($typo == 'read') {
                    $mail->read = 1;
    ...
    public function newmailsent(Request $request)
    {
        try {
            ...
            if ($type == 'sendit') {
                $cator = Tag::byType('mailcat')->where('slug', 'sent')->first()->id;
                $catorurl = "/admin/mailbox/inbox";
                $catormessage = trans("admin.emailsended");
            } elseif ($type == 'draftsave') {
                $cator = Tag::byType('mailcat')->where('slug', 'drafts')->first()->id;
